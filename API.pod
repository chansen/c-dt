...
=head2 dt_from_easter
    ...
    int dt_from_easter(int year, dt_computus_t computus);

Returns a date value corresponding to Easter Sunday for the given I<year>. The 
parameter I<computus> specifies the Easter calculation.

I<(computus = DT_WESTERN)> Calculation based on the Gregorian calendar.

I<(computus = DT_ORTHODOX)> Calculation based on the Julian calendar with 
the Julian date converted to the equivalent Gregorian date.
...
Returns a date value corresponding to next or current day of week for the given 
date value I<dt>. The parameter I<dow> specifies the day of week (1=Monday to 7=Sunday).
The parameter I<current> specifies whether or not the returned date value must succeed
the given date value I<dt>.

B<Example:>

    /* 2012-W01-1 */
    dt1 = dt_from_ywd(2012, 1, DT_MONDAY);

    /* 2012-W01-1 (dt2 == dt1) */
    dt2 = dt_next_day_of_week(dt1, DT_MONDAY, true);

    /* 2012-W02-1 */
    dt3 = dt_next_day_of_week(dt1, DT_MONDAY, false);

=head2 dt_prev_day_of_week

    int dt_prev_day_of_week(int dt, int dow, bool current);

Returns a date value corresponding to previous or current day of week for the given 
date value I<dt>. The parameter I<dow> specifies the day of week (1=Monday to 7=Sunday).
The parameter I<current> specifies whether or not the returned date value must precede
the given date value I<dt>.
...
Returns a date value corresponding to I<delta> months added the given date value I<dt>.
The parameter I<adjust> defines the behaviour when the resulting month has fewer
days than the day of the month component for the given date. 

I<(adjust = DT_EXCESS)> If the resulting month has fewer days than the day of 
month component for the given date, then the result is counted forward or backward 
into the next or previous month by the number of excessive days. Otherwise, the 
result has the same day of month component as given date. For example:
...
month component for the given date, then the result is the last day of the 
resulting month. Otherwise, the result has the same day of month component as 
...
resulting month has fewer days than the day of month component for the given date, 
then the result is the last day of the resulting month. Otherwise, the result has 
the same day of month component as given date. For example:
