...
The test harness is written in C using the L<Test Anything Protocol|http://en.wikipedia.org/wiki/Test_Anything_Protocol>. 
Perl and the L<prove|http://search.cpan.org/dist/Test-Harness/> command is required 
to run the harness. The version of I<prove> must be equal or greater than 3.23.
